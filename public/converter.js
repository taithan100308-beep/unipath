...
// Mapping tên file PDF cho V-SAT
const vsatPdfFiles = {
  toan: 'vsat/toan.pdf',
  ngu_van: 'vsat/ngu_van.pdf',
  vat_li: 'vsat/vat_li.pdf',
  hoa_hoc: 'vsat/hoa_hoc.pdf',
  dia_li: 'vsat/dia_li.pdf',
  lich_su: 'vsat/lich_su.pdf',
  tieng_anh: 'vsat/tieng_anh.pdf',
  sinh_hoc: 'vsat/sinh_hoc.pdf'
};
...
function switchTab(tabName) {
  ...
  if (tabName === 'vsat') {
    document.getElementById('vsat-tab').classList.add('active');
    event.target.classList.add('active');
  } else if (tabName === 'dgnl') {
    document.getElementById('dgnl-tab').classList.add('active');
    event.target.classList.add('active');
  }
}
...
function convertScoreVSAT() {
  const subject = document.getElementById("vsatSubject").value;
  const x = parseFloat(document.getElementById("vsatScore").value);
  const ranges = vsatData[subject];
...
function convertScoreDGNL() {
  const x = parseFloat(document.getElementById("dgnlScore").value);

  if (isNaN(x)) {
    document.getElementById("resultDGNL").innerHTML = "❌ Vui lòng nhập điểm hợp lệ!";
    return;
  }

  const range = dgnlData.find((r) => x > r.a && x <= r.b);
  if (!range) {
    document.getElementById("resultDGNL").innerHTML = "⚠️ Điểm ngoài phạm vi quy đổi!";
    return;
  }

  const y = interpolate(x, range.a, range.b, range.c, range.d);
  document.getElementById("resultDGNL").innerHTML = `🔹 Điểm THPT tương đương: <b>${y.toFixed(2)}</b>`;
}

// ============= XEM PDF V-SAT =============

function loadVSATPdf() {
  const subject = document.getElementById("vsatPdfSelect").value;
  const pdfViewer = document.getElementById("vsatPdfViewer");

  if (!subject) {
    pdfViewer.innerHTML = `
      <div class="pdf-placeholder">
        <p>📄 Vui lòng chọn môn để xem đề minh họa</p>
      </div>
    `;
    return;
  }

  const pdfPath = vsatPdfFiles[subject];
  pdfViewer.innerHTML = `<iframe src="${pdfPath}#zoom=80"></iframe>`;
}
